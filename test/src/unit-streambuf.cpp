...
TEST_CASE("Consume packages from empty buffer", "[streambuf]") {
    ...
    auto consumed = streambuf.consume_packages(packages, error);
    REQUIRE(consumed == 0);
    REQUIRE(!error);
}
...
TEST_CASE("Consume incomplete buffer sequentially", "[streambuf]") {
    ...
    consumed = streambuf.consume_packages(packages, error);
    REQUIRE(consumed == 0);
    REQUIRE(!error);
